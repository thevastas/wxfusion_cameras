...
bool AutoFeaturesManager::IsPixelFormatSupported(const peak::ipl::Image& image)
{
    ...
    else if (("Mono" == image.PixelFormat().Name().substr(0, 4)) && (BalanceWhiteAutoMode::Off == m_balanceWhiteAutoMode))
    {
        return true;
    }
    else
...
uint8_t AutoFeaturesManager::CalculateBrightnessAutoMedian(const peak::ipl::Image& image)
{
    ...
    if ((median - 1) > 0)
    {
        m_brightnessAutoMedian = median - 1;
    }
    else
    {
        m_brightnessAutoMedian = 0;
        ...
        || ((median - m_brightnessAutoTarget) > m_brightnessAutoTargetTolerance))
    {
        ...
        try
        {
            ...
            double currentGain = 1.0;
            {
                ...
                currentGain = m_nodemapRemoteDevice->FindNode<peak::core::nodes::FloatNode>("Gain")->Value();
...
void AutoFeaturesManager::CalculateBalanceWhiteAuto()
{
    ...
    int bpp = static_cast<int>(image.PixelFormat().CalculateStorageSizeOfPixels(1));
...
std::shared_ptr<peak::core::DataStream> NIRCam::OpenDevice()
{
...
cv::UMat NIRCam::GetFrame(bool automaticmode, std::shared_ptr<peak::core::DataStream> dataStream)
{
    ...
    auto image = peak::BufferTo<peak::ipl::Image>(buffer).ConvertTo(peak::ipl::PixelFormatName::BGR8, peak::ipl::ConversionMode::Fast);
